...
关内用「下一页」翻这五块；翻完才到下一课。宽屏上 Live2D 助教叠在同一张羊皮纸上，不单独占一列；完整五块笔记用「详细笔记」打开，不挡脸。
...
旁边「看不懂？」打开词语小抄：BPE、号码牌、练习卷/验收卷、右移一格、猜错罚分。BPE 是把词切碎的另一种办法；这一课不用，一个字符一张号码牌。「详细笔记」在游戏区内打开，不盖住助教的脸。电脑端还有游戏指针，会跟着悬停和点击变样子。
