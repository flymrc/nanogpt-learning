...
- **目录** lists 5 chapters (纸带和号码 / 剪开猜下一个 / 只看左边 / 按错题分拧旋钮 / 接着往后写字). Short buttons stay 纸带 / 猜下一个 / 看左边 / 改一改 / 往后写. The in-lesson header for chapter 4 is 按罚分改一笔. Every chapter starts at beat 0 and can be revisited. **返回** steps to the previous phase, then the previous beat, then the previous chapter.
