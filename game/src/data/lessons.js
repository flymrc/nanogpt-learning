...
const L5 = [
  {
    ...
    mood: "point",
    note: "sample",
    goal: "往后续字，就是读已经存好的进度，从一小段开头接上新字。大人把这一步叫做采样。这一课不再拧旋钮。",
    why: "上一章按错题分把旋钮拧好，验收更好才存档。要看见新格子，得另走这一步。它不把练习卷当成要抄的答案。",
    example: "默认开头是一个换行，号码是 0。也可以换成你写的一小段。本仓库没有存档，这里不展示编出来的台词。",
    myths: ["不是把练习卷再抄一遍。", "不是验收卷上的错题分。"],
    remember: "往后续字：从开头接新格子。旋钮不再拧。",
    footnote: "存档的文件名写在详细笔记里。续字前先停住「练习时随机盖住一点」的习惯。我们这里没有这份存档。",
  },
  {
    id: "loop",
    purpose: "猜一格，接上，再猜",
    ...
    why: "它一次不会交出整段台词。新格子会变成下一次能看见的左边。这和上一章「看见前面、猜下一个」是同一件事，只是没有标准答案。",
  ...
  },
  {
    ...
    myths: ["温度不是拧旋钮的步子。往后续字的时候，旋钮已经不拧了。", "不是这一课真的砍掉了很多字。65 个字全都留着。"],
  ...
  },
  {
    ...
    mood: "point",
    note: "sample",
    goal: "续字要去读那份存档，再把号码翻回字。电脑上要打的字，写在伪代码和详细笔记里。",
    why: "文件夹的名字必须和拧旋钮时一样。找不到对照表，就会换一套切法，字的号码就对不上。",
    example: "那一行去读存档，再用对照表把号码翻回字。本仓库没有这份存档。说明里的示例台词是作者的，不是我们跑出来的。",
    myths: ["不是这台电脑已经印出了莎士比亚。", "不是另一套更大的课。"],
    remember: "记住的是：去读存档，把号码翻回字。",
    footnote: `只有普通电脑时，说明里另写了让这台电脑自己算。抽签的固定起点是 ${SAMPLE.seed}。本课没有跑出台词。`,
  },
  {
    id: "score",
    purpose: "续字不是考试分数",
    ...
    why: "验收错题分问的是：没拿来拧旋钮的那一段里，真答案被押了多少。续字的时候没有那一格的真答案，所以没有罚分。好看的句子也不能代替那次抽查。",
    example: "上一章里，验收错题分变小才把存档留下来。续字只是把那份存档读出来接字。读的时候不拧旋钮，也不打印一个准确率。",
  ...
  },
  {
    ...
    mood: "react",
    note: "sample",
    goal: "纸带、右移一格和猜错罚分、只看左边、按错题分拧旋钮、从开头续字。这五步，你已经能自己讲完。",
    why: "讲完不等于这台电脑已经拧过旋钮，也不等于已经印出台词。存档不在这里。每一块里后来那次混合，拧旋钮时也会动，但我们没有单独拆开讲。",
    example: "若要离开这 65 个字：还有一条路会把词语切成小碎块，以及八台电脑一起算的大课。那是另一条纸带，号码牌不是这一课的 65 张。",
    myths: ["不是电脑已经会写剧本。", "不是还差一个正确率才算讲完。"],
    remember: "准备、右移罚分、只看左边、拧旋钮、续字。这条课走到这里。",
...
export const TITLE_BEAT = {
  ...
  vo: "vo-title",
  mood: "talk",
  goal: "顺着看完：纸带和号码，剪开猜下一个，只看左边，按错题分拧旋钮，接着往后写字。",
...
export const END_BEAT = {
  ...
  vo: "vo-clear",
  mood: "react",
  goal: "你现在能自己讲完：纸带怎么拉，答案怎么右移，罚分怎么看，每一格怎么只看左边，怎么按错题分拧旋钮，续字怎么往后续。",
