...
关内用「下一步细节」翻这五块；翻完才到下一拍。宽屏右侧留给 Live2D 助教；完整五块笔记用左上角「详细笔记」在游戏区打开，不挡脸。
...
旁边「看不懂？」打开词语小抄：BPE、号码牌、练习卷/验收卷、往右挪一格、罚分。**GPT-2 会用 BPE**；这一课不用 BPE，而是一个字符一个号码。「详细笔记」在游戏区内打开抽屉，不盖住宽屏 Live2D 助教。电脑端还有游戏指针，会跟着悬停和点击变样子。
