#!/usr/bin/env python3
"""Generate original BGM, tap SFX, and lesson VO clips for the Phaser game.

BGM / SFX are synthesized here (original, CC0).
Lesson VO uses edge-tts, then ffmpeg silenceremove + loudnorm + 80k mono mp3.
Chinese voice: zh-CN-XiaoxiaoNeural, rate -8%, pitch +6Hz.
Japanese voice: ja-JP-NanamiNeural, rate -8%, pitch +0Hz.

    python3 game/scripts/generate-audio.py
    node game/scripts/generate-lesson-vo.mjs
"""
...
# Legacy stems in public/audio (vo-title, vo-level1, …) are not the kid script.
# Lesson lines are generated by generate-lesson-vo.mjs into public/audio/vo/.
VO_LINES = {}
LESSON_VOICES = {
    "zh": {"voice": "zh-CN-XiaoxiaoNeural", "rate": "-8%", "pitch": "+6Hz"},
    "ja": {"voice": "ja-JP-NanamiNeural", "rate": "-8%", "pitch": "+0Hz"},
}
...
def normalize_voice(src: Path, wav_path: Path) -> None:
    subprocess.run(
        [
            "ffmpeg",
            "-y",
...
async def synth_with_retry(text: str, raw_mp3: Path, voice: str, rate: str, pitch: str) -> None:
    last = None
    for attempt in range(5):
        try:
            await synth_vo_edge(text, raw_mp3, voice, rate, pitch)
            if raw_mp3.exists() and raw_mp3.stat().st_size > 200:
                return
            last = RuntimeError(f"empty mp3 for {raw_mp3.name}")
        except Exception as exc:  # noqa: BLE001 — retry transient edge-tts errors
            last = exc
        await asyncio.sleep(1.5 * (attempt + 1))
    raise RuntimeError(f"edge-tts failed for {raw_mp3.name}: {last}")


async def render_lesson_job(job: dict, sem: asyncio.Semaphore) -> None:
    async with sem:
        lang = job["lang"]
        spec = LESSON_VOICES[lang]
        out = Path(job["out"])
        if out.exists() and out.stat().st_size >= 1000:
            print(f"  keep {lang}/{out.name}", flush=True)
            return
        stem = f"{lang}-{job['hash']}"
        raw_mp3 = TMP / f"{stem}-raw.mp3"
        wav_norm = TMP / f"{stem}.wav"
        print(f"  synth {lang}/{job['key']}", flush=True)
        await synth_with_retry(job["text"], raw_mp3, spec["voice"], spec["rate"], spec["pitch"])
        await asyncio.to_thread(normalize_voice, raw_mp3, wav_norm)
        await asyncio.to_thread(encode_mp3, wav_norm, out)
        print(f"  wrote {lang}/{out.name} {out.stat().st_size} B", flush=True)


async def render_lesson_vo(jobs_path: Path) -> None:
    jobs = json.loads(jobs_path.read_text(encoding="utf-8"))
    if TMP.exists():
        shutil.rmtree(TMP)
    TMP.mkdir(parents=True)
    sem = asyncio.Semaphore(4)
    await asyncio.gather(*(render_lesson_job(job, sem) for job in jobs))
    shutil.rmtree(TMP, ignore_errors=True)
    print(f"lesson vo clips: {len(jobs)}")


def main() -> None:
    if "--lesson-vo" in sys.argv:
        idx = sys.argv.index("--lesson-vo")
        asyncio.run(render_lesson_vo(Path(sys.argv[idx + 1])))
        return
